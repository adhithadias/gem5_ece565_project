    ...
    const PacketPtr pkt)
{
    ...
    std::shared_ptr<LRUReplData> dip_replacement_data =
        std::static_pointer_cast<LRUReplData>(replacement_data);

    bool winner = !duelingMonitor.getWinner();
    bool team;
    bool is_sample = duelingMonitor.isSample(pkt->getAddr(), team);
    ...
    if (is_sample) {
        if (team) {
            LRURP::reset(dip_replacement_data);
        } else {
            BIPRP::reset(dip_replacement_data);
        }
        // replacementPolicy1->reset(
                // dip_replacement_data->replacementData1, pkt);
        // replacementPolicy2->reset(
                // dip_replacement_data->replacementData2, pkt);
    } else {
        if (winner) { // true then select BIP policy PSEL is high
            LRURP::reset(dip_replacement_data);
            // replacementPolicy2->reset(
                    // dip_replacement_data->replacementData1, pkt);
            // replacementPolicy2->reset(
                // dip_replacement_data->replacementData2, pkt);
        } else { // winner if false PSEL is low select LRU policy
            // replacementPolicy1->reset(
                    // dip_replacement_data->replacementData1, pkt);
            // replacementPolicy1->reset(
                    // dip_replacement_data->replacementData2, pkt);
...
DIPRP::reset(const std::shared_ptr<ReplacementData>& replacement_data) const
{
    ...
    // duelingMonitor.sample(pkt->getAddr());
    // duelingMonitor.sample(dip_replacement_data->replacementData1.);
...
//         "support team sizes that match the "
...
//         dip_replacement_data.push_back(dip_repl_data);
        // candidate->replacementData = team_a ?
        //     dip_repl_data->replacementData1 :
